...
function CurrentPreferenceSelector({ menuId }: CurrentPreferenceSelectorProps) {
  ...
  return (
    <section>
      <h2 className="text-2xl font-semibold text-gray-900">{t('moodTitle')}</h2>
      <p className="mt-2 text-gray-500">{t('moodText')}</p>
      {group(t('protein'), preferredProteinOptions, selectedProteins, setSelectedProteins)}
      {group(t('flavour'), preferredFlavourOptions, selectedFlavours, setSelectedFlavours)}
      {group(t('texture'), preferredTextureOptions, selectedTextures, setSelectedTextures)}
      <div className="mt-6">
        <h3 className="text-sm font-semibold text-gray-700">{t('spiceLevel')}</h3>
        <div className="mt-3 flex flex-wrap gap-3">
      ...
      <button type="button" onClick={handleRecommend} disabled={loading} className="mt-8 w-full rounded-xl bg-green-700 px-6 py-3 font-medium text-white disabled:opacity-60">
        {loading ? t('updating') : t('recommend')}
      </button>
      {recommendations.length > 0 && (
        <div className="mt-10 space-y-4">
          <h2 className="text-2xl font-bold text-gray-900">{t('topPicks')}</h2>
