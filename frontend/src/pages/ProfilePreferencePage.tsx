    ...
    useEffect(() => {
        ...
        getPreferenceProfile()
            .then((profile) => {
                ...
                setSelectedReligiousRestrictions(profile.religious_restrictions)
                setSelectedPreferredProteins(profile.preferred_proteins)
                setSelectedPreferredFlavours(profile.preferred_flavours)
                setSelectedPreferredTextures(profile.preferred_textures)
                setSelectedSpiceLevel(profile.spice_level)
                setDislikedIngredients(profile.disliked_ingredients)
            })
            .catch((reason) => {
                if (active) setError(reason instanceof Error ? reason.message : 'Could not load preferences.')
            })
            .finally(() => {
...
const handleSavePreferences = async () => {
    ...
    setSaving(true)
    try {
        await savePreferenceProfile(preferenceProfile)
        onContinue()
    } catch (reason) {
...
return (
    ...
    <div className="mx-auto max-w-2xl">
        ...
        {loading && <p className="mt-5 text-sm text-gray-500">Loading saved preferences...</p>}
        ...
        <button
            ...
            className="mt-10 w-full rounded-xl bg-green-700 px-6 py-3 font-medium text-white hover:bg-green-800 disabled:opacity-60"
        >
            {saving
                ? 'Saving...'
                : isOnboarding
