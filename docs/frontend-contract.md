...
image_url
image_is_reference
enrichment.display_summary
evidence.explicit_ingredients
evidence.reference_ingredients
evidence.inferred_ingredients
evidence.allergen_assessments
evidence.display.explicit_ingredients
evidence.display.reference_ingredients
evidence.display.inferred_ingredients
decision.status
decision.reasons
decision.warnings
```
...
Use `evidence.display.*` for user-facing ingredient text. On `dish`, the
`explicit_ingredients` field preserves the menu evidence,
`canonical_ingredients_en` supports deterministic matching, and
`translated_explicit_ingredients` is display-only. These fields must not
overwrite one another. Fall back to the canonical value only when an older
saved response has no display field.

Use `enrichment.display_summary` before the cached English `summary`. This
prevents an English fallback description from leaking into a Chinese or French
result card.
...
## Language, evidence and Demo presentation

The frontend supports English, Chinese, and French UI dictionaries. A language
selected on login is saved to `profile.preferred_language`; live and Demo menu
responses then use the same language for translated dish content and
recommendation reasons.

Price display must prefer `dish.price_text`, which preserves source formatting
such as `€18,50` and `€14/24`. Only use `price + currency` as a fallback.

Detailed cards keep evidence visually separate:

```text
explicit_ingredients    restaurant-menu fact
reference_ingredients   generic reference recipe
inferred_ingredients    AI or cached culinary inference
allergen_assessments    deterministic mapping with an evidence source
display                 localized copies used only by the UI
```

The safety disclaimer is displayed once per results page, not once per dish.
Individual cards show concrete reasons and expose further evidence behind a
details control.
