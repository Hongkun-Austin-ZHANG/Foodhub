...
`canonical_guess` is only for English database matching. Translation fields are
only for frontend display. `explicit_ingredients` contains only restaurant-menu
facts. Validate B JSON with `POST /api/menu/validate`.
