...
def test_backend_b_payload_preserves_translation_and_multi_price() -> None:
            ...
            "dishes": [
                {
                    ...
                    "canonical_ingredients_en": [
                        "snail",
                        "garlic and parsley crumbs",
                    ],
                    "translated_explicit_ingredients": ["蜗牛", "蒜香欧芹面包屑"],
    ...
    assert menu.dishes[0].translated_explicit_ingredients == [
        "蜗牛",
        "蒜香欧芹面包屑",
    ]
    assert menu.dishes[0].canonical_ingredients_en == [
        "snail",
        "garlic and parsley crumbs",
    ]
    assert menu.dishes[0].price is None
    assert menu.dishes[0].price_text == "14/24"
    assert menu.dishes[0].currency == "EUR"
