...
class FakeMenuParser:
    ...
    async def parse_many(
        ...
        return await self.parse(images[0][0], images[0][1], target_language)
...
async def test_image_pipeline_uses_profile_language_fallback_and_cache(
    ...
    assert first_body["dishes"][0]["enrichment"]["display_summary"] == (
        "该菜品的补充信息来自本地菜品知识库。"
    )
    assert first_body["dishes"][0]["evidence"]["display"]["inferred_ingredients"] == [
        {
            "name": "黄油",
            "reasoning": "根据常见烹饪方式推断该成分可能出现。",
        }
    ]
    assert "milk" not in first_body["dishes"][0]["decision"]["reasons"][0]
    assert "牛奶/乳制品" in first_body["dishes"][0]["decision"]["reasons"][0]
    assert first_body["fallback_batch_request"] is None
    assert parser.target_languages == ["zh", "zh"]
    assert len(fallback.calls) == 1
    assert second.status_code == 200
    assert second.json()["dishes"][0]["resolution_status"] == "local_fallback"
    assert second.json()["dishes"][0]["enrichment"]["display_summary"] == (
        "该菜品的补充信息来自本地菜品知识库。"
    )
...
@pytest.mark.asyncio
async def test_demo_scan_uses_database_template_without_live_dependencies(
    api_client: AsyncClient,
) -> None:
    registration = await register_user(api_client)
    headers = {"Authorization": f"Bearer {registration['access_token']}"}
    ...
    scan = await api_client.post("/api/menu/demo-scan", headers=headers)
    ...
    onion_soup = body["dishes"][0]
    assert onion_soup["dish"]["translated_name"] == "法式洋葱汤"
    assert onion_soup["dish"]["price_text"] == "€18,50"
    assert onion_soup["enrichment"]["source"] == "demo_database"
    assert onion_soup["enrichment"]["cuisine"] == "French"
    assert onion_soup["enrichment"]["display_cuisine"] == "法国菜"
    assert onion_soup["evidence"]["explicit_ingredients"] == [
        "onion broth",
        "aged Gruyère",
        "croutons",
    ]
    assert onion_soup["evidence"]["display"]["explicit_ingredients"] == [
        "洋葱高汤",
        "陈年格鲁耶尔奶酪",
        "面包丁",
    ]
    assert onion_soup["decision"]["status"] == "avoid"
    ...
    recommendations = await api_client.post(
        ...
        json={
            ...
            "current_preference": {"preferred_textures": ["crispy"]},
        },
    )
    assert recommendations.status_code == 200
    assert recommendations.json()["mode"] == "demo"
