...
from services.demo_menu import build_demo_analysis
...
async def persist_menu_analysis(
    result: RecommendationAnalyzeResponse,
    user_id: str,
    session: SessionDep,
) -> None:
    session.add(
...
async def scan_menu(
    ...
    await persist_menu_analysis(result, user.id, session)
    return result
...
@router.post(
    "/demo-scan",
    response_model=RecommendationAnalyzeResponse,
    summary="Load the sanitized database-backed demonstration menu",
)
async def demo_scan_menu(
    user: CurrentUserDep,
    session: SessionDep,
) -> RecommendationAnalyzeResponse:
    settings = get_settings()
    if not settings.demo_available:
        raise HTTPException(
    ...
    target_language = profile.preferred_language if profile is not None else "en"
    result = await build_demo_analysis(user.id, target_language, session)
    await persist_menu_analysis(result, user.id, session)
    return result
