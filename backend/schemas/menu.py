...
class BackendBDishPayload(BaseModel):
    ...
    canonical_ingredients_en: list[str] = Field(default_factory=list)
    translated_explicit_ingredients: list[str] = Field(default_factory=list)
    ...
    def to_parsed_dish(self) -> ParsedDish:
        price, price_text = parse_menu_price(self.price)
        confidence = (
        ...
        return ParsedDish(
            ...
            canonical_ingredients_en=self.canonical_ingredients_en,
            translated_explicit_ingredients=self.translated_explicit_ingredients,
